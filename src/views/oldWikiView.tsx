...
interface OldWikiPageViewProps {
    aboutLayer : AboutLayer,
    wikiLayer : WikiLayer,
    pageDetail : WikiPageDetail | null,
}
interface OldWikiPageViewState {
    isEditing : boolean;
    editedText : string;
}
let logPage = (...args : any[]) => console.log('OldWikiPageView |', ...args);
export class OldWikiPageView extends React.Component<OldWikiPageViewProps, OldWikiPageViewState> {
    constructor(props : OldWikiPageViewProps) {
        super(props);
        this.state = {
            isEditing: false,
            editedText: '',
...
let logWiki = (...args : any[]) => console.log('OldWikiView |', ...args);
export class WikiView extends React.Component<WikiViewProps, WikiViewState> {
    ...
    componentDidMount() {
        logWiki('subscribing to storage onChange');
        this.props.aboutLayer.storage.onChange.subscribe(() => {
            logWiki('onChange =============');
            this.forceUpdate()
        });
    }
